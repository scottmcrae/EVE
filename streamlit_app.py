...
def fetch_systems():
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT DISTINCT system_name FROM public.market_spread_jita_view WHERE system_name IS NOT NULL ORDER BY system_name")
...
def build_haul_table(df):
    tid  = "tbl-haul"
    rows = ""
    for i, (_, r) in enumerate(df.iterrows()):
        jumps     = int(r["jumps"]) if r["jumps"] is not None else "—"
        name_safe = str(r["product"]).replace('"', "&quot;")
        rows += (
            f'<tr class="{"top-row-whale" if i < 3 else ""}"'
            f' data-name="{name_safe.lower()}" data-jumps="{r["jumps"] if r["jumps"] is not None else 999}"'
            f' data-margin="{abs(float(r["margin"]))*100}" data-profit="{r["total_profit"]}"'
            f' data-pps="{r["profit_per_shipload"]}" data-investment="{r["investment"]}">'
            f'<td>{r["product"]}</td>'
            f'<td>{fmt(r["sell_price"])}</td>'
            f'<td>{fmt(r["buy_price"])}</td>'
            f'<td class="{haul_margin_cls(r["margin"])}">{abs(float(r["margin"]))*100:.1f}%</td>'
            f'<td class="{haul_jumps_cls(r["jumps"])}">{jumps}</td>'
            f'<td>{fmt(r["total_profit"])}</td>'
            f'<td>{fmt(r["profit_per_shipload"])}</td>'
            f'<td>{fmt(r["investment"])}</td>'
            f'<td>{r["goods_volume"]:,.0f}</td>'
    ...
    hdr = (
        ...
        f'<th style="cursor:pointer" onclick="sortTable(\'{tid}\',\'buy\',this)">Buy <span class="si"></span></th>'
        f'<th style="cursor:pointer" onclick="sortTable(\'{tid}\',\'margin\',this)">Margin <span class="si"></span></th>'
        f'<th style="cursor:pointer" onclick="sortTable(\'{tid}\',\'jumps\',this)">Jumps <span class="si"></span></th>'
        f'<th style="cursor:pointer" onclick="sortTable(\'{tid}\',\'profit\',this)">Total profit <span class="si">▼</span></th>'
        f'<th style="cursor:pointer" onclick="sortTable(\'{tid}\',\'pps\',this)">Profit/shipload <span class="si"></span></th>'
        f'<th style="cursor:pointer" onclick="sortTable(\'{tid}\',\'investment\',this)">Investment <span class="si"></span></th>'
        f'<th style="cursor:pointer" onclick="sortTable(\'{tid}\',\'vol\',this)">Goods vol <span class="si"></span></th>'
...
def render_tier(tier_df, tier, tab):
    with tab:
        ...
        # add cost column for sorting
        filtered["cost"] = filtered["capturable"] * filtered["buy_price"]
...
with st.spinner("Loading hauling data..."):
    try:
        ...
        if haul_df.empty:
            ...
            hcol_sort = st.selectbox("Sort by ", ["Total profit", "Profit/shipload", "Margin", "Jumps", "Investment"], key="haul_sort")
            hcol_map  = {"Total profit": "total_profit", "Profit/shipload": "profit_per_shipload", "Margin": "margin", "Jumps": "jumps", "Investment": "investment"}
            haul_df["margin"] = pd.to_numeric(haul_df["margin"], errors="coerce").fillna(0)
            haul_df = haul_df.sort_values(hcol_map[hcol_sort], ascending=(hcol_sort == "Jumps"))
            st.markdown(build_haul_table(haul_df), unsafe_allow_html=True)
    except Exception as e:
