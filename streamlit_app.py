...
def build_haul_table(df, cargo_capacity=10000):
    tid  = "tbl-haul"
    rows = ""
    for i, (_, r) in enumerate(df.iterrows()):
        ...
        rows += (
            ...
            f' data-unitprofit="{profit}" data-pps="{r["profit_per_shipload"]}">' +
            f'<td>{r["product"]}</td>' +
...
render_tier(whale_df, "whale")
render_tier(mid_df,   "mid")
render_tier(vol_df,   "vol")

# ── Hauling Opportunities ──────────────────────────────────────────────────
_hc1, _hc2 = st.columns([4, 2])
with _hc1:
    st.markdown("""
...
with _hc2:
    ...
    cargo_capacity = st.number_input("Cargo Capacity (m³)", min_value=0, value=10000, step=500, key="cargo_cap")
...
try:
    ...
    if haul_df.empty:
        ...
        hcol_sort = st.selectbox("Sort by ", ["Margin", "Jumps"], key="haul_sort")
        hcol_map  = {"Margin": "margin", "Jumps": "jumps"}
