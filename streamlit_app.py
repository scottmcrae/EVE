...
def build_table(df, tier="whale"):
    ...
    for i, (_, r) in enumerate(df.iterrows()):
        ...
        rows += (
            ...
            f'<td>{r["type_name"]}</td>'
            f'<td class="{buy_cls(r["buy_price"])}">Ƶ {fmt(r["buy_price"])}</td>'
            f'<td>Ƶ {fmt(r["spread"])}</td>'
            f'<td class="{mg_cls(r["margin_pct"], tier)}">{r["margin_pct"]:.1f}%</td>'
            f'<td>{r["asv"]:,.0f}</td>'
            f'<td>{r["abv"]:,.0f}</td>'
            f'<td>{sold}</td>'
            f'<td class="{isk_cls(r["daily_isk"], tier)}">Ƶ {fmt(r["daily_isk"] * 1_000_000)}</td>'
            f'<td>Ƶ {fmt(cost)}</td>'
            f'</tr>'
        )
    hdr = (
...
def build_haul_table(df, cargo_capacity=6500, capital=100_000_000, tax_rate=5.02):
    ...
    for i, (_, r) in enumerate(df.iterrows()):
        ...
        items       = min(cap_items, cargo_items)
        ...
        rows += (
            ...
            f' data-items="{items}" data-mktitems="{int(r["goods_volume"])}">' +
            f'<td>{r["product"]}</td>' +
            f'<td>Ƶ {fmt(sell_p)}</td>' +
            f'<td>Ƶ {fmt(buy_p)}</td>' +
            ...
            f'<td class="isk-high">Ƶ {fmt(adj_profit)}</td>' +
...
# ── Header ─────────────────────────────────────────────────────────────────
col_logo, col_sys, col_btn = st.columns([5, 2, 1])
with col_logo:
    st.markdown("""
...
try:
    ...
    if haul_df.empty:
        ...
        def calc_capital_profit(r):
            ...
            items  = min(int(capital / sell_p), int(cargo_capacity / vol) if vol > 0 else 0)
