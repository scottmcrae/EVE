...
def fetch_last_update():
    ...
    try:
        ...
        if row and row[0] is not None:
            return f"{float(row[0]):.1f} hours ago"
        return "—"
    finally:
        conn.close()
...
def fetch_finished_at():
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT TO_CHAR(
                    (NOW()::date::text || ' ' || current_time::text)::timestamptz AT TIME ZONE 'America/Chicago',
                    'Day HH12:MIam'
...
&nbsp;|&nbsp; LAST UPDATE: <span style="color:#c8d4e0">{last_update}</span> &nbsp;|&nbsp; FINISHED AT: <span style="color:#c8d4e0">{finished_at}</span>
